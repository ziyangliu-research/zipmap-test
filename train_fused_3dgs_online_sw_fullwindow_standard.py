#!/usr/bin/env python3
"""
online sliding-window full-window joint loss
每个 optimizer step 同时 render 当前 window 内所有 views
total_loss.backward()
optimizer.step()

目前暂时还用不到，但是后续可以测试损失相加之后再优化的这个策略
"""
"""
Online-style STANDARD full-window sliding-window refinement for a fused ReSplat/MVSplat Gaussian map
...
First target setting:
  Full-window loss: each optimizer step can use all cameras in the current window.
  In --current_batch_mode all, all window-view losses are summed/averaged first,
  then a single backward() and optimizer.step() are executed. This is the standard
  joint-loss version, not memory-saving gradient accumulation.
...
def training_online_sliding(dataset, opt, pipe, args):
    ...
    if args.densification_mode != "off":
        raise ValueError("This online full-window sliding-window prototype currently supports only --densification_mode off")
    ...
    for wid, start_pos, win_ids, new_ids in windows:
        ...
        if args.eval_before_each_window:
            extra = {
                ...
                "current_batch_mode": args.current_batch_mode,
        ...
        for local_iter in pbar:
            ...
            # ------------------------------------------------------------------
            # Current-window loss.
            # random: previous 3DGS-style stochastic behavior, one camera/step.
            # all   : full-window loss, all current window cameras contribute to
            #         the same optimizer step via gradient accumulation.
            # ------------------------------------------------------------------
            if args.current_batch_mode == "random":
                if not viewpoint_stack:
                    viewpoint_stack = list(cams_window)
                rand_idx = randint(0, len(viewpoint_stack) - 1)
                current_cams = [viewpoint_stack.pop(rand_idx)]
            elif args.current_batch_mode == "all":
                current_cams = list(cams_window)
            else:
                raise ValueError(f"Unknown current_batch_mode: {args.current_batch_mode}")
            ...
            # Standard joint window loss:
            #   render all current-window cameras first,
            #   average their losses into one tensor,
            #   then call backward() once below.
            for viewpoint_cam in current_cams:
                (view_loss, Ll1, ssim_value, Ll1depth_item, image, gt_image, render_pkg) = render_mapping_loss_for_camera(
                    viewpoint_cam, scene.gaussians, pipe, bg, dataset, opt, depth_l1_weight, global_iteration
                )
                current_losses.append(view_loss)
                current_loss_items.append(float(view_loss.detach().item()))
                current_l1_items.append(float(Ll1.detach().item()))
                current_ssim_items.append(float(ssim_value.detach().item()) if hasattr(ssim_value, "item") else float(ssim_value))
                current_depth_items.append(float(Ll1depth_item))

            if current_losses:
                current_loss_tensor = torch.stack(current_losses).mean()
            else:
                current_loss_tensor = torch.tensor(0.0, device="cuda")

            current_loss_scalar = sum(current_loss_items) / max(1, len(current_loss_items))
            Ll1_scalar = sum(current_l1_items) / max(1, len(current_l1_items))
            ssim_scalar = sum(current_ssim_items) / max(1, len(current_ssim_items))
            Ll1depth_item = sum(current_depth_items) / max(1, len(current_depth_items))

            # Replay loss. In full-window mode this behaves like MonoGS-style
            # current-window + past-keyframe loss, but still uses the original
            # GraphDECO renderer/GaussianModel.
            ...
            if replay_cams:
                ...
                for replay_cam in replay_cams:
                    ...
                    replay_loss_items.append(float(r_loss.detach().item()))
            if replay_losses:
                replay_loss_tensor = torch.stack(replay_losses).mean()
            else:
                replay_loss_tensor = torch.tensor(0.0, device="cuda")
            replay_loss_scalar = sum(replay_loss_items) / max(1, len(replay_loss_items)) if replay_loss_items else 0.0

            if args.lambda_iso > 0:
                iso_loss = active_isotropic_loss(scene.gaussians, active_mask)
                iso_loss_scalar = float(iso_loss.detach().item())
            else:
                iso_loss = torch.tensor(0.0, device="cuda")
                iso_loss_scalar = 0.0

            total_loss = current_loss_tensor + args.lambda_replay * replay_loss_tensor + args.lambda_iso * iso_loss
            total_loss.backward()
            ...
            with torch.no_grad():
                ema_loss_for_log = 0.4 * total_loss_scalar + 0.6 * ema_loss_for_log
                ema_depth_for_log = 0.4 * Ll1depth_item + 0.6 * ema_depth_for_log
                if local_iter % 10 == 0:
                    pbar.set_postfix({
                        "Loss": f"{ema_loss_for_log:.6f}",
                        "CurN": f"{len(current_cams)}",
                        "Replay": f"{replay_loss_scalar:.5f}",
                        "Iso": f"{iso_loss_scalar:.5f}",
                        "#G": f"{scene.gaussians.get_xyz.shape[0]}",
                        "active": f"{active_count}",
                    })

                if tb_writer:
                    tb_writer.add_scalar("train_loss/l1", Ll1_scalar, global_iteration)
                    tb_writer.add_scalar("train_loss/total", total_loss_scalar, global_iteration)
                ...
                if wandb_run is not None and global_iteration % args.wandb_log_interval == 0:
                    sampled_psnr = psnr(image.detach(), gt_image.detach()).mean().double().item() if image is not None else float("nan")
                    wandb_run.log({
                        "train/l1_current_mean": Ll1_scalar,
                        "train/ssim_current_mean": ssim_scalar,
                        "train/psnr_last_view": sampled_psnr,
                        "train/loss_total": total_loss_scalar,
                        "train/loss_current": current_loss_scalar,
                        "train/loss_replay": replay_loss_scalar,
                        "train/loss_iso": iso_loss_scalar,
                        "train/num_current_views": len(current_cams),
                ...
                if use_sparse_adam:
                    if args.current_batch_mode == "all":
                        raise RuntimeError("SparseAdam is not supported with --current_batch_mode all in this prototype; use default optimizer.")
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Online-style full-window sliding-window fused 3DGS refinement")
    ...
    parser.add_argument("--current_batch_mode", choices=["random", "all"], default="all",
                        help="random = one random current-window camera per step; all = all current-window cameras are rendered, losses are averaged, and a single backward/step is executed.")
