...
# -*- coding: utf-8 -*-
"""
P7-B: render-supervised local opacity + footprint-shrink optimization for incremental 3DGS map maintenance.
...
It is deliberately conservative:
  ...
  - Only old opacity values and a bounded shrink factor for covariance/scale are optimized.

Recommended first run
---------------------
Use 25-35 first, no P4:

python render_supervised_opacity_shrink_update_p7b.py \
  ...
  --lr_opacity 0.05 \
  --lr_shrink 0.02 \
  --optimize_params opacity_shrink \
  --loss_mode l1 \
  --loss_mask_mode none \
  --opacity_cap 0.30 \
...
def make_gaussians(
    ...
    covariances_override: Optional[torch.Tensor] = None,
    ...
    for k in ALL_GAUSSIAN_KEYS:
        ...
        elif k == "covariances" and covariances_override is not None:
            val = covariances_override
        elif k == "scales" and scales_override is not None:
            val = scales_override
...
def render_rgb(
    ...
    gaussians = make_gaussians(
        ...
        fields,
        gaussian_keys,
        opacities_override=opacities_override,
        covariances_override=covariances_override,
    ...
    color = out.color[0, 0].float()
    # Important: do NOT clamp during optimization; clamp can kill gradients in saturated regions.
    return color.clamp(0, 1) if clamp_output else color
...
def shrink_logits_to_factor(logits: torch.Tensor, min_shrink: float) -> torch.Tensor:
    # Factor is constrained to [min_shrink, 1].
    # Initial logits are large positive, so factor starts close to 1.
    min_s = float(min_shrink)
    return min_s + (1.0 - min_s) * torch.sigmoid(logits)
...
def optimize_old_opacity_for_current_view(
    ...
    optimize_opacity = args.optimize_params in {"opacity", "opacity_shrink"}
    optimize_shrink = args.optimize_params in {"shrink", "opacity_shrink"}

    # Baseline render and optional loss mask. Use unclamped color for loss.
    with torch.no_grad():
        rendered0 = render_rgb(Gaussians_cls, decoder, fields, current_packet, device, gaussian_keys, H, W, clamp_output=False)
    loss_mask = make_loss_mask(rendered0, target, args)

    base_opacities = fields["opacities"].detach().clone()
    base_covariances = fields["covariances"].detach().clone()
    base_scales = fields["scales"].detach().clone() if "scales" in fields else None

    params = []
    opt_init = base_opacities[opt_mask].detach().clone()
    logits_opacity = None
    if optimize_opacity:
        logits_opacity = torch.nn.Parameter(
            opacities_to_logits(torch.clamp(opt_init, min=0.0, max=float(args.opacity_cap)), args.opacity_cap)
        )
        params.append({"params": [logits_opacity], "lr": float(args.lr_opacity)})

    shrink_logits = None
    if optimize_shrink:
        # Start very close to 1.0, so the first render is almost unchanged.
        shrink_logits = torch.nn.Parameter(
            torch.full((nopt,), float(args.shrink_init_logit), device=device, dtype=torch.float32)
        )
        params.append({"params": [shrink_logits], "lr": float(args.lr_shrink)})

    if not params:
        raise ValueError("--optimize_params must include opacity and/or shrink")
    ...
    shrink_final_for_meta = None

    for it in range(int(args.num_steps)):
        optimizer.zero_grad(set_to_none=True)

        op_override = base_opacities
        if optimize_opacity:
            opt_values = logits_to_opacities(logits_opacity, args.opacity_cap)
            op_override = base_opacities.clone()
            op_override[opt_mask] = opt_values
        else:
            opt_values = opt_init

        cov_override = base_covariances
        scales_override = base_scales
        if optimize_shrink:
            shrink_factor = shrink_logits_to_factor(shrink_logits, args.min_shrink)
            cov_override = base_covariances.clone()
            cov_override[opt_mask] = base_covariances[opt_mask] * (shrink_factor[:, None, None] ** 2)
            if base_scales is not None:
                scales_override = base_scales.clone()
                scales_override[opt_mask] = base_scales[opt_mask] * shrink_factor[:, None]
        else:
            shrink_factor = None
        ...
        rendered = render_rgb(
            ...
            covariances_override=cov_override,
            scales_override=scales_override,
            clamp_output=False,
        )
        ...
        if optimize_opacity and args.opacity_l1 > 0:
            loss = loss + float(args.opacity_l1) * opt_values.mean()
        if optimize_opacity and args.opacity_l2_delta > 0:
            loss = loss + float(args.opacity_l2_delta) * ((opt_values - opt_init) ** 2).mean()

        if optimize_shrink:
            # Keep shrink close to 1 unless the render loss really benefits.
            loss = loss + float(args.shrink_l1) * (1.0 - shrink_factor).mean()
            loss = loss + float(args.shrink_l2_delta) * ((1.0 - shrink_factor) ** 2).mean()
        ...
        if args.grad_clip > 0:
            opt_params_for_clip = []
            if logits_opacity is not None:
                opt_params_for_clip.append(logits_opacity)
            if shrink_logits is not None:
                opt_params_for_clip.append(shrink_logits)
            torch.nn.utils.clip_grad_norm_(opt_params_for_clip, float(args.grad_clip))

        optimizer.step()
        loss_final_val = float(loss_rgb.detach().item())

        if optimize_shrink:
            shrink_final_for_meta = shrink_logits_to_factor(shrink_logits, args.min_shrink).detach()
        ...
        if args.verbose_opt and ((it + 1) % max(1, int(args.print_every)) == 0 or it == 0):
            ...
            if optimize_opacity:
                msg += f", opacity_mean={float(opt_values.detach().mean().item()):.6f}"
            if optimize_shrink:
                sf = shrink_logits_to_factor(shrink_logits, args.min_shrink).detach()
                msg += f", shrink_mean={float(sf.mean().item()):.6f}, shrink_min={float(sf.min().item()):.6f}"
            print(msg)

    with torch.no_grad():
        fields["opacities"] = fields["opacities"].detach().clone()
        if optimize_opacity:
            opt_final = logits_to_opacities(logits_opacity, args.opacity_cap).detach()
        ...
        if optimize_shrink:
            shrink_final = shrink_logits_to_factor(shrink_logits, args.min_shrink).detach()
            fields["covariances"] = fields["covariances"].detach().clone()
            fields["covariances"][opt_mask] = base_covariances[opt_mask] * (shrink_final[:, None, None] ** 2)
            fields["covariances"] = fields["covariances"].contiguous()
            if "scales" in fields:
                fields["scales"] = fields["scales"].detach().clone()
                fields["scales"][opt_mask] = base_scales[opt_mask] * shrink_final[:, None]
                fields["scales"] = fields["scales"].contiguous()
        else:
            shrink_final = torch.ones((nopt,), device=device)
    ...
    meta = {
        ...
        "lr_shrink": float(args.lr_shrink),
        ...
        "shrink_mean_after": float(shrink_final.mean().item()),
        "shrink_min_after": float(shrink_final.min().item()),
        "shrink_max_after": float(shrink_final.max().item()),
        "shrink_changed_mean_abs": float(torch.abs(1.0 - shrink_final).mean().item()),
        **mask_meta,
    }
    return fields, meta
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--lr_shrink", type=float, default=0.02)
    p.add_argument("--optimize_params", choices=["opacity", "shrink", "opacity_shrink"], default="opacity_shrink")
    ...
    p.add_argument("--min_shrink", type=float, default=0.3)
    p.add_argument("--shrink_init_logit", type=float, default=8.0)
    p.add_argument("--shrink_l1", type=float, default=0.001)
    p.add_argument("--shrink_l2_delta", type=float, default=0.001)
...
def main() -> None:
    ...
    print("[3/5] Incremental insert -> opacity/shrink optimization -> optional P4")
    ...
    for step_i, idx in enumerate(selected):
        ...
        print(
            ...
            f"shrink={p7_meta.get('shrink_mean_after', None)}, "
    ...
    meta = {
        "script": "render_supervised_opacity_shrink_update_p7b.py",
    ...
    summary_path = out_dir / "p7b_opacity_shrink_update_summary.json"
    with summary_path.open("w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2, ensure_ascii=False)
    write_csv(out_dir / "p7b_opacity_shrink_update_steps.csv", step_rows)
    write_csv(out_dir / "p7b_opacity_shrink_update_per_packet.csv", per_packet_rows)

    print(f"      saved packet: {args.output_pt}")
    print(f"      saved summary: {summary_path}")
    print(f"      saved steps csv: {out_dir / 'p7b_opacity_shrink_update_steps.csv'}")
    print(f"      saved per-packet csv: {out_dir / 'p7b_opacity_shrink_update_per_packet.csv'}")
