...
def draw_camera_triangle(
    ...
    """Draw a clear isosceles triangle marker for a camera pose."""
    cx, cy = center_uv
    if dir_uv is None:
        # Default upward orientation in image space.
        ux, uy = 0.0, -1.0
    else:
    ...
    base_center = (cx - 0.55 * size * ux, cy - 0.55 * size * uy)
    left = (base_center[0] + 0.65 * size * px, base_center[1] + 0.65 * size * py)
    right = (base_center[0] - 0.65 * size * px, base_center[1] - 0.65 * size * py)
    draw.polygon([tip, left, right], fill=fill, outline=outline)
    # Draw center dot to make it visible even when the triangle is small.
    rr = max(2, size // 5)
    draw.ellipse([cx - rr, cy - rr, cx + rr, cy + rr], fill=(255, 255, 255, 230))
...
def overlay_trajectory_on_image(
    ...
    for i, idx in enumerate(overlay_indices):
        ...
        draw_camera_triangle(
            draw,
            (uu, vv),
            dir_uv,
            size=int(args.overlay_triangle_size),
            fill=fill,
    ...
    if current_packet_index in overlay_indices:
        ...
        if bool(valid_z[j]) and np.isfinite(u) and np.isfinite(v):
            if in_loose_image(float(u), float(v), W, H, margin) or args.overlay_clamp_to_image:
                ...
                dir_uv = None
                if valid2[0] and valid2[1] and np.all(np.isfinite(uv2)):
                    du, dv = float(uv2[1, 0]), float(uv2[1, 1])
                    if args.overlay_clamp_to_image:
                        du, dv, _ = clamp_point_to_image(du, dv, W, H, pad=int(args.overlay_edge_pad))
                    dir_uv = (du, dv)
                draw_camera_triangle(
                    draw,
                    (uu, vv),
                    dir_uv,
                    size=int(args.overlay_current_triangle_size),
                    fill=current_color,
...
def main() -> None:
    ...
    parser.add_argument("--overlay_triangle_color", type=int, nargs="+", default=[255, 0, 0, 255], help="Color for camera triangle markers.")
    parser.add_argument("--overlay_triangle_size", type=int, default=16, help="Triangle marker size for trajectory cameras.")
    parser.add_argument("--overlay_current_triangle_size", type=int, default=26, help="Triangle marker size for the current camera.")
    parser.add_argument("--overlay_no_line", action="store_true", help="Do not draw trajectory lines; draw camera triangles only.")
    parser.add_argument("--overlay_clamp_to_image", action="store_true", default=True, help="Clamp out-of-frame projected camera markers to image border so they remain visible.")
