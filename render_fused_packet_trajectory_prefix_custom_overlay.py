...
def project_world_points(points_w: np.ndarray, render_packet: Dict[str, Any], extrinsic_type: str) -> Tuple[np.ndarray, np.ndarray]:
    T_raw = get_pose_matrix(render_packet)
    Tcw = tcw_from_extrinsic(T_raw, extrinsic_type)
    K = first_camera_matrix(render_packet["target_intrinsics"], (3, 3)).detach().cpu().numpy().astype(np.float64)
...
def overlay_trajectory_on_image(
    ...
    uv, valid_z = project_world_points(centers_np, render_packet, args.extrinsic_type)
    ...
    for i, idx in enumerate(overlay_indices):
        if i % every != 0:
            continue
        u, v = uv[i]
        if not (bool(valid_z[i]) and np.isfinite(u) and np.isfinite(v) and in_loose_image(float(u), float(v), W, H, margin)):
            continue
        r = int(args.overlay_point_radius)
        draw.ellipse([u - r, v - r, u + r, v + r], fill=point_color, outline=(0, 0, 0, 180))

    # Viewing arrows.
    arrow_every = max(1, int(args.overlay_arrow_every))
    for i, idx in enumerate(overlay_indices):
        if i % arrow_every != 0:
            continue
        c = centers_np[i]
        p1 = c + float(args.overlay_arrow_len) * forwards[i]
        pts = np.stack([c, p1], axis=0)
        uv2, valid2 = project_world_points(pts, render_packet, args.extrinsic_type)
        if not (valid2[0] and valid2[1] and np.all(np.isfinite(uv2))):
            continue
        if not in_loose_image(float(uv2[0, 0]), float(uv2[0, 1]), W, H, margin):
            continue
        draw_arrow(draw, (float(uv2[0, 0]), float(uv2[0, 1])), (float(uv2[1, 0]), float(uv2[1, 1])), arrow_color, int(args.overlay_arrow_width))

    # Highlight current camera.
    if current_packet_index in overlay_indices:
        j = list(overlay_indices).index(current_packet_index)
        u, v = uv[j]
        if bool(valid_z[j]) and np.isfinite(u) and np.isfinite(v) and in_loose_image(float(u), float(v), W, H, margin):
            r = int(args.overlay_current_radius)
            draw.ellipse([u - r, v - r, u + r, v + r], fill=current_color, outline=(255, 255, 255, 230), width=2)
            c = centers_np[j]
            p1 = c + float(args.overlay_arrow_len) * forwards[j]
            pts = np.stack([c, p1], axis=0)
            uv2, valid2 = project_world_points(pts, render_packet, args.extrinsic_type)
            if valid2[0] and valid2[1] and np.all(np.isfinite(uv2)):
                draw_arrow(draw, (float(uv2[0, 0]), float(uv2[0, 1])), (float(uv2[1, 0]), float(uv2[1, 1])), current_color, max(2, int(args.overlay_arrow_width) + 1))

    if args.overlay_legend:
        text = f"trajectory {overlay_indices[0]}-{overlay_indices[-1]} | current {current_packet_index}"
        pad = 6
        bbox = draw.textbbox((0, 0), text)
        tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
        draw.rectangle([8, 8, 8 + tw + 2 * pad, 8 + th + 2 * pad], fill=(0, 0, 0, 130))
        draw.text((8 + pad, 8 + pad), text, fill=(255, 255, 255, 235))
...
def main() -> None:
    ...
    parser.add_argument("--fused_packet_pt", type=Path, default=None, help="Required for --render_mode trajectory/custom_pose. Not used for custom_pose_prefix.")
    ...
    # Render camera mode.
    parser.add_argument("--render_mode", choices=["trajectory", "custom_pose", "custom_pose_prefix"], default="trajectory",
                        help="trajectory: render one fused map along packet target views; custom_pose: render one fused map at one fixed custom camera; custom_pose_prefix: read packets and render each prefix-fused map at one fixed custom camera.")
    ...
    # Overlay options.
    parser.add_argument("--overlay_trajectory", action="store_true", help="Draw trajectory and camera direction on each rendered image.")
    ...
    for out_i, (packet_i, path, packet, frame_idx, prefix_indices) in enumerate(render_jobs):
        ...
        overlay_path = None
        if args.overlay_trajectory:
            # In custom_pose_prefix, image k should show only the camera path up to current packet.
            # In trajectory/custom_pose mode, draw the user-specified overlay_indices.
            overlay_now = prefix_indices if (args.render_mode == "custom_pose_prefix" and prefix_indices is not None) else overlay_indices
    ...
    summary = {
        ...
        "num_views": len(rows),
        "custom_pose_prefix_final_num_gaussians": rows[-1]["num_gaussians"] if rows and args.render_mode == "custom_pose_prefix" else None,
