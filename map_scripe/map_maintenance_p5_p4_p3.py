...
# -*- coding: utf-8 -*-
"""
P5: integrated P4 redundancy control + P3 trajectory/depth/extent safety filtering.
...
P5 combines them in one script so that the map is loaded/fused only once for
...
def run_incremental(
    ...
    for step_i, idx in enumerate(selected_indices):
        ...
        map_fields, map_source, map_uid, p4_meta, p4_seen = apply_p4(map_fields, map_source, map_uid, args, p4_seen)
        map_fields, map_source, map_uid, p3_meta, p3_seen = apply_p3(map_fields, map_source, map_uid, views, args, p3_seen)
        ...
        row_step.update({f"p3_{k}": v for k, v in p3_meta.items() if isinstance(v, (int, float, str, bool)) or v is None})
        step_rows.append(row_step)

        print(
            f"[Incremental {step_i+1:03d}/{len(selected_indices):03d}] "
            f"packet={idx:04d}, after_insert={n_after_insert:,}, after_maintenance={n_after:,}, "
            f"p3_views={len(views)}"
        )
