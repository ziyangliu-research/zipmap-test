...
V3 overlay changes: visible-camera-only by default, outline/transparent frustum-like triangles, optional direction flip.
New feature:
  --overlay_trajectory
...
def overlay_trajectory_on_image(
    ...
    for i, idx in enumerate(overlay_indices):
        ...
        draw_camera_triangle(
            ...
            fill=fill,
            outline=rgba(args.overlay_triangle_outline_color, 230),
            outline_width=int(args.overlay_triangle_outline_width),
            fill_enabled=bool(args.overlay_triangle_fill),
            flip_direction=bool(args.overlay_flip_triangle),
        )
        drawn += 1
        ...
        if bool(valid_z[j]) and np.isfinite(u) and np.isfinite(v):
            if in_loose_image(float(u), float(v), W, H, margin) or args.overlay_clamp_to_image:
                ...
                draw_camera_triangle(
                    ...
                    fill=current_color,
                    outline=rgba(args.overlay_current_outline_color, 255),
                    outline_width=max(2, int(args.overlay_triangle_outline_width) + 1),
                    fill_enabled=bool(args.overlay_triangle_fill),
                    flip_direction=bool(args.overlay_flip_triangle),
                )
