#!/usr/bin/env python3
# single-view sliding window + random replay views
"""
Online-style sliding-window refinement with replay/isotropic regularization for a fused ReSplat/MVSplat Gaussian map
...
Online/incremental semantics:
  ...
  - Optimize only Gaussians whose source packet is inside the current window.
  - Previously inserted but out-of-window Gaussians stay frozen, but still render/occlude.
...
def training_online_sliding(dataset, opt, pipe, args):
    ...
    for wid, start_pos, win_ids, new_ids in windows:
        ...
        active_mask = active_mask_for_packets(inserted_packet_order, win_ids, gpp, "cuda")
        ...
        print(f"\n[W{wid}] inserted={inserted_packet_order[0]}-{inserted_packet_order[-1]} ({len(inserted_packet_order)} packets), "
              f"window={win_ids[0]}-{win_ids[-1]}, active_gaussians={active_count:,}, total_gaussians={scene.gaussians.get_xyz.shape[0]:,}, "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trainable_mode", choices=list(TRAINABLE_GROUPS.keys()), default="opacity_color_scale")
